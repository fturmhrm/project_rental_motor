...
@section('content')
    <div class="container p-5 text-center">
        <h1 class="mb-5">Daftar Rental Motor</h1>
        <button class="btn btn-primary mb-2">
            <a href="{{ route('rental.tambah_data') }}" class="text-white">Tambah Data</a>
        </button>
            ...
            <tbody>
                ...
                @else
                    @foreach ($rentals as $index => $item)
                        <tr>
                            <td>{{ ($rentals->currentPage() - 1) * $rentals->perPage() + ($index + 1) }}</td>
                            <td>{{ $item->name }}</td>
                            <td>{{ $item->type }}</td>
                            <td>{{ $item->hours }}</td>
                            <td class="text-center">
                                <a href="{{ route('rental.edit', $item->id) }}" class="btn btn-primary">
                ...
                <form id="form-delete-data" method="POST">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Hapus Data Rental</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            Apakah anda yakin ingin menghapus data rental <span id="nama-rental"></span>?
                        </div>
...
@push('script')
    ...
    <script>
        function showModal(id, name) {
            let urldelete = '{{ route('rental.delete', ':id') }}';
            urldelete = urldelete.replace(':id', id);
            $('#form-delete-data').attr('action', urldelete);
            $('#exampleModal').modal('show');
            $('#nama-rental').text(name);
        }
    </script>
@endpush
